...
class FrontController extends AppController
{
	...
	public $uses=array('ApiXml','User','Product','News','MetaTitle','sendEmail','Val','Black','Banner','Quotejai','Payment','JadwalPray','Promo', 'Linequiz');
	...
	public function beforeFilter() {
		...
		if(!$this->Auth->loggedIn()) $this->Auth->deny('premium_payment','claim');  //action apa saja yang harus login terlebih dahulu
		...
		if($this->Session->read('Auth.User.role')=='admin') 	$this->redirect(array('controller' => 'adm', 'action' => 'logout'));
	...
	public function form_promotion(){
		...
		} else {throw new NotFoundException('Could not find that page');}
	}
	...
	public function response_purchase(){
		...
		if($this->request->is('post') && isset($this->request->data['klikPayCode'])){
			...
			try{
				if(substr($data_sprint['transactionNo'],0,1)=='J') { // JAI
					...
					if(count($rescount)==1) {
						if($data_sprint['klikPayCode']!=$rescount['Payment']['klikPayCode']) {
							...
							$id=2;CakeLog::write('error', $transactionNo.' '.$transactionDate.' Total Amount tidak cocok');
						}
						else if($authKey!=$rescount['Payment']['authKey']) {
							$id=2;CakeLog::write('error', $transactionNo.' '.$transactionDate.' authKey tidak cocok');
						}
							...
							array('approvalCode'=>"'".$approvalCode."'",'status'=>"'".$status."'",'reason_id'=>"'".$id."'",'reason_ind'=>"'Sukses'",'reason_en'=>"'Success'"),
							array('transactionNo'=>$data_sprint['transactionNo']));
						}
					}
					else {$id=2; CakeLog::write('error', $transactionNo.' '.$transactionDate.' transactionNo dan transactionDate tidak cocok');}

					if($id==2){
						$this->Payment->updateAll(
						array('reason_id'=>"'".$id."'",'reason_ind'=>"'Transaksi Anda tidak dapat diproses.'",'reason_en'=>"'Your transaction cannot be processed.'"),
						array('transactionNo'=>$data_sprint['transactionNo']));
					} else if($id==3){
						$this->Payment->updateAll(
						array('reason_id'=>"'".$id."'",'reason_ind'=>"'Transaksi Anda sudah dibayar.'",'reason_en'=>"'Your transaction has been paid.'"),
						array('transactionNo'=>$data_sprint['transactionNo']));
					}
				}
				...
				else $id=$this->ApiXml->updatePaymentKlikBCAPremi($this->request->data); //utk pembayaran 2
			} catch(Exception $e){
				$id=7;
	...
	public function cal_non_unitlink_ajax(){
		...
		if($this->request->is('post') && $this->request->is('ajax')){
			...
			$this->set(compact('data','valjai','premium'));
		} else throw new NotFoundException('Could not find that page');
	}
	...
	public function check_email(){
		...
		if($this->request->is('ajax') && $this->Session->check('Purchase.premi')){
			...
			if(is_numeric($id_prospect)) echo "1";
  ...
  public function check_valid_dns_email(){
    $this->autoRender=false;
      ...
      if($verify->verify($email_valid)) echo '1'; else echo '2';
  ...
  public function checkAcumulation(){
    ...
    if($this->request->is('ajax')){
      ...
      if(($getAcumulate['TotalSumInsured']+$total_insured) <= $getAcumulate['MaxSumInsured']) echo "1";
      } else echo "1";
    }
    else throw new NotFoundException('Could not find that page');
  }
	...
	public function getBCAtoken($name=''){
		...
		$id = $this->request->query['id'];
		if($this->request->is('ajax') && $this->Session->read('Purchase.QUOTE_ID')==$id){
			if($this->Session->read('Purchase.step1.product_id')!=7) $quote=$this->ApiXml->getQuoteByID($id);
			else {
			...
			$totalAmount=number_format($this->Session->read('Purchase.premi.total_premi'), 2, '.', '');
	...
	public function getECashtoken($name=''){
		...
		if($this->request->is('ajax') && $this->Session->read('Purchase.QUOTE_ID')==$id){
			...
			if($this->Session->read('Purchase.step1.product_id')==7){
				...
				$this->Session->write('Purchase.idPolicy', $idPolicy);
				$this->Session->write('Purchase.step1.product_id',7);
			}  else $idCafEcash= $this->ApiXml->savePaymentEcash($return_url,$amount,$description,$quote['QuoteNo']);
	...
	public function getVisaMastertoken($name=''){
		...
		if($this->request->is('ajax') && $this->Session->read('Purchase.QUOTE_ID')==$id){
			...
			$amount=$this->Session->read('Purchase.premi.total_premi');
			$siteID="jagadiri";
			$serviceVersion="1.0";
		...
		if($this->Session->check('Purchase.id_BCA') || $this->Session->check('Purchase.id_BCApremi')){
			try{
				if($this->Session->check('Purchase.id_BCA')) {
					...
					} else $status=$this->ApiXml->getPaymentKlikBCA($this->Session->read('Purchase.id_BCA'));
				}
				else $status=$this->ApiXml->getPaymentKlikBCAPremi($this->Session->read('Purchase.id_BCApremi'));
		...
		if($this->Session->check('Purchase.id_ecash')){
			try{
				...
				$status=trim($result[4]);
					...
					} else $status=$this->ApiXml->updateEcash($result[3],$res,$status);
	...
	public function step2_your_detail($name=""){
		...
		if ($this->ApiXml->getValidUrl($sid,$cat,$name)){
			...
			$prodListID = array(11, 12, 13);
			if((in_array($this->Session->read('Purchase.step1.product_id'),$prodListID)) && $arrayID==null) $arrayID=array('1');
		...
		if(!$this->Auth->loggedIn()) $this->set('captcha_fields', $this->captchas);
	}
	...
	public function step4_checkout(){
        ...
        if(!isset($sess['me'])) $sess['me']='Y';
			...
			if($cat=='unit-link') $chart=$this->ApiXml->GetGrafikUnitLink($this->Session->read('Purchase.QUOTE_ID')); else $chart=null;
			...
			$this->set(compact('sid','cat','name','chart','_metaTitle','urlKlikPay','urlDO','ECashPayUrl','optInsureRel'));
		}
		else{
			$this->redirect("/");
		}
		...
		$this->set(compact('emph','embir','phbir','email_black','phone_black','birth_black'));
	}
	...
	public function step2_add_ta($name=""){
		...
		if ($this->ApiXml->getValidUrl($sid,$cat,$name)){
			if($this->request->is('post') || $this->request->is('put')  ){
				...
				if(!isset($this->request->data['Tertanggung']['PROSPECT_GENDER']) || $this->request->data['Tertanggung']['PROSPECT_GENDER']==null) $this->request->data['Tertanggung']['PROSPECT_GENDER'] = $this->Session->read('Purchase.step1.PROSPECT_GENDER');
				...
				if($searchTU!=-1) {$this->Session->write('Purchase.Tertanggung.'.$searchTU, $aw);}
	...
	public function step2_del_aw($name=""){
		...
		if ($this->ApiXml->getValidUrl($sid,$cat,$name)){
			...
			$i=1;$n=1;
			while($i<=5){
				if(isset($temp[$i])){$newData[$n]=$temp[$i]; $n++;}
				$i++;
			}
			$this->Session->delete('Purchase.Ahliwaris');
    ...
    $promo=$this->Promo->find('all',array('conditions'=>array('deleted'=>0,'Promo.start_date <= ' => $date,'Promo.end_date >= ' => $date,)));
		$this->set(compact('checkProd','news','allproduct','ctleavenumber','banners','allphone','promo'));
	}
	...
	public function landing(){
		...
		if($productdetail==null) 	throw new NotFoundException('Could not find that page');
  ...
  $promo=$this->Promo->find('all',array('conditions'=>array('deleted'=>0,'Promo.start_date <= ' => $date,'Promo.end_date >= ' => $date,)));
    ...
    #Show promo image with date conditions
    $date = date("Y-m-d H:i:s");
    $promo=$this->Promo->find('all',array('conditions'=>array('Promo.id !='=>3,'deleted'=>0,'Promo.start_date <= ' => $date,'Promo.end_date >= ' => $date,)));
	...
	public function product(){
		...
		$this->set(compact('acc','life','health','investa'));
	}
	...
	public function temukansolusi(){
		...
		if($emph==null){
			if(isset($this->request->data['Contactme'])){
				...
				if(!isset($this->request->data['Product']['question1'])){$q1=0;};
				if(!isset($this->request->data['Product']['question2'])){$q2=0;};
				if(!isset($this->request->data['Product']['question3'])){$q3=0;};
				//if user select then fill the value
				if($q1!=0){$q1=$this->request->data['Product']['question1'];};
				if($q2!=0){$q2=$this->request->data['Product']['question2'];};
				if($q3!=0){$q3=$this->request->data['Product']['question3'];};
  ...
  public function linequiz_index(){
  	/**
	* Line Quiz is temporarely closed until further noticed
	* disabled on: 16 Nov 2015
  	*/
  	$this->redirect('/');
  ...
  public function linequiz_send(){
	...
	if($this->request->is('post')) {
		...
		else CakeLog::write('error', $customer_id); // write API Error to log
		...
		else CakeLog::write('error', $benef_id); // write API Error to log

		 if(isset($customer_cert_id)) { // Success
			$this->Linequiz->updateAll(
			array('step_stts'=>2,'customer_id'=>$customer_id, 'certificate_id'=>$customer_cert_id),
			array('cookie_id'=>$linequiz_session));
		 }
