...
class NicepayLib {
    ...
    public function merchantTokenF() {
        // SHA256( Concatenate(iMid + customerId + merchantKey) )
        return hash('sha256',   $this->get('iMid').
            $this->get('customerId').
            $this->merchantKey
        );
    ...
    public function merchantTokenFx() {
        // SHA256( Concatenate(iMid + startDt + vacctNo + merchantKey) )
        return hash('sha256',   $this->get('iMid').
            $this->get('customerId').
            $this->get('startDt').
            $this->merchantKey
    ...
    public function merchantTokenFv() {
        // SHA256( Concatenate(iMid + startDt + vacctNo + merchantKey) )
        return hash('sha256',   $this->get('iMid').
            $this->get('vacctNo').
            $this->get('startDt').
...
//       Send Request
    ...
    public function registerCustomerId($customerId, $customerNm) {
        // Populate data
        $this->set('iMid', $this->iMid);
        $this->set('customerId', $customerId);
        $this->set('customerNm', $customerNm);
        $this->set('merchantToken', $this->merchantTokenF());

        // Check Parameter
        $this->checkParam('iMid', '01');
        $this->checkParam('merchantToken', '28');
        $this->checkParam('customerId', '31');
        $this->checkParam('customerNm', '32');
        // Send Request
        $this->request->operation('registerCustomerId');
        $this->request->openSocket();
        $this->resultData = $this->request->apiRequest($this->requestData);
        unset($this->requestData);
        return $this->resultData;
    ...
    public function retrieveVaInfo($customerId) {
        // Populate data
        $this->set('iMid', $this->iMid);
        $this->set('customerId', $customerId);
        $this->set('merchantToken', $this->merchantTokenF());
        ...
        $this->checkParam('customerId', '31');
        // Send Request
        $this->request->operation('retrieveVaInfo');
        $this->request->openSocket();
        $this->resultData = $this->request->apiRequest($this->requestData);
        unset($this->requestData);
        return $this->resultData;
    ...
    public function listDepositCustomerId($customerId,$startDt,$endDt) {
        // Populate data
        $this->set('iMid', $this->iMid);
        $this->set('customerId', $customerId);
        $this->set('startDt', $startDt);
        $this->set('endDt', $endDt);
        $this->set('merchantToken', $this->merchantTokenFx());

        // Check Parameter
        $this->checkParam('iMid', '01');
        $this->checkParam('customerId', '31');
        $this->checkParam('startDt', '34');
        $this->checkParam('endDt', '35');
        $this->checkParam('merchantToken', '28');
        // Send Request
        $this->request->operation('listDepositCustomerId');
        $this->request->openSocket();
        $this->resultData = $this->request->apiRequest($this->requestData);
        unset($this->requestData);
        return $this->resultData;
    ...
    public function listDepositVa($vacctNo,$startDt,$endDt) {
        // Populate data
        $this->set('iMid', $this->iMid);
        $this->set('vacctNo', $vacctNo);
        $this->set('startDt', $startDt);
        $this->set('endDt', $endDt);
        $this->set('merchantToken', $this->merchantTokenFv());

        // Check Parameter
        $this->checkParam('iMid', '01');
        $this->checkParam('vacctNo', '33');
        $this->checkParam('startDt', '34');
        $this->checkParam('endDt', '35');
        $this->checkParam('merchantToken', '28');
        // Send Request
        $this->request->operation('listDepositVa');
        $this->request->openSocket();
        $this->resultData = $this->request->apiRequest($this->requestData);
        unset($this->requestData);
        return $this->resultData;
    ...
    public function bankCd2Bank($bankCd) {
        switch ($bankCd) {
            case 'CENA':
                $bank = 'Bank BCA';
                break;
    ...
    public function getError($id)
    {
        $error = array(
            ...
            '33' =>   array(
                'error'    => '10033',
                'errorMsg' => '(vacctNo) is not set. Please set (vacctNo).'
            ),
            '34' =>   array(
                'error'    => '10034',
                'errorMsg' => '(startDt) is not set. Please set (startDt).'
            ),
            '35' =>   array(
                'error'    => '10035',
                'errorMsg' => '(endDt) is not set. Please set (endDt).'
            )
