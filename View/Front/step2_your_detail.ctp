...
<div class="row margintop">
			<div class="col-md-12">
				...
				<span class="bold">Lengkap data diri Anda</span>
...
<div class="row">
	...
	<div class="form-group">
		...
		<div class="form-group">
		<label class="col-sm-2 control-label">No KTP<span class="red">*</span></label>
			<div class="col-sm-9">
			<div>
...
<div class="modal fade" id="modalaw" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog ">
          ...
          <div class="form-group">
            ...
            <div class="col-md-7">
              <?php echo $this->Form->input('PROSPECT_MOBILE_PHONE', array('required'=>'required','type'=>'text','class'=>'form-control')); ?>
            </div>
          </div>
          ...
          <div class="alert alert-warning alert-dismissable">
            ...
            <span class="red">Jika ahli waris berusia kurang dari 17 tahun</span>, harap gunakan email WALI dari ahli waris (sangat disarankan keluarga yang tidak serumah). Wali akan menerima informasi pertanggungan dan berhak mewakili pengurusan klaim ahli waris. <span class="red">Jika ahli waris di atas 17 tahun</span>, harap masukkan data email Ahli Waris.
	  </div>
		...
		<div class="modal-dialog ">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group" id="InsuredNRIC" >
						<label class="col-md-3 control-label">No KTP</label>
						<div class="col-md-7">
							<?php echo $this->Form->input('PROSPECT_NRIC', array('required'=>'false', 'class'=>'form-control' )); ?>
						</div>
					</div>
	...
	$.ajax({
      ...
      <?php endif;?>
			postData("<?php echo $this->Html->url(array('controller'=>'front','action'=>'step2_add_ta','id'=>$name,'?'=>$this->request['url'])); ?>",{
				...
				"data[Tertanggung][PROSPECT_NRIC]":document.getElementById("DetailPROSPECTNRIC").value,
...
var valQuote = $("#formQuote").validate({
 ...
 focusInvalid:false,
	rules: {
		...
		"data[Detail][PROSPECT_NRIC]" :{ required:true, ktp:true},
	...
	messages: {
		...
		"data[Detail][PROSPECT_NRIC]": {required:"Masukan nomor KTP",ktp:"No KTP yang Anda masukkan tidak valid"},
...
}, "No KTP yang Anda masukkan tidak valid");

jQuery.validator.addMethod("ktp_no", function(value, element) {
	if(/^[0-9]*$|\./.test( value ) ){return true;}else{return false;} 
}, "No KTP yang Anda masukkan tidak valid");
...
jQuery.validator.addMethod("myEmail", function(value, element) {
    ...
    return this.optional( element ) || ( /^[a-z0-9]+([-._][a-z0-9]+)*@([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,4}$/.test( value ) && /^(?=.{1,64}@.{4,64}$)(?=.{6,100}$).*/.test( value ) 
	&& (value .endsWith(".com") || value .endsWith(".co") || value .endsWith(".org") || value .endsWith(".net") ||value .endsWith(".tv") || value .endsWith(".me") || value .endsWith(".id") || value .endsWith(".ac.id") || value .endsWith(".biz.id")|| value .endsWith(".co.id")|| value .endsWith(".my.id")|| value .endsWith(".or.id")|| value .endsWith(".sch.id")|| value .endsWith(".web.id")  ) );
...
var FormAH = $("#FormAH").validate({
 ...
 focusInvalid:false,
	messages: {
		...
		"data[Ahliwaris][PROSPECT_MOBILE_PHONE]": "Masukan Nomor Telepon Ahli Waris Anda",
...
$('#tadobanak').change(function() {
if(getAge($('#tadobanak').val() ) >= 17) {
    //alert("You have 17 or more years old!");
	$("#InsuredNRIC").show();

}else{ 
    //alert($('#tadobanak').val() );
	$("#InsuredNRIC").hide();
	$("#TertanggungPROSPECTNRIC").attr("required", false);
}

});

$('#tadob').change(function() {
if(getAge($('#tadob').val() ) >= 17) {
    //alert("You have 17 or more years old!");
	$("#InsuredNRIC").show();
