...
<div class="row">
  <div class="col-md-12">
    ...
    <div class="box-grey-second">
      ...
      <h4 class="title-calculate"><span class="bold"><?php $basic_premium = $prem_total-0; ?> <?php echo rp($basic_premium); if($basic_premium<=0){ $this->Session->destroy();?>
<script>if (window.confirm('Terjadi kesalahan, mohon ulangi proses pembelian hanya dengan 1 tab saja pada browser anda.'))
{
    // They clicked Yes
...
else
{
	...
	window.location = "<?php echo $this->Html->url('/')?>";
          ...
          <?php else: ?>
            <h4 class="title-calculate"><span class="bold"><?php $basic_premium = $prem_total; ?><?php echo rp($basic_premium); if($basic_premium<=0){session_destroy();?>
<script>if (window.confirm('Terjadi kesalahan, mohon ulangi proses pembelian hanya dengan 1 tab saja pada browser anda.'))
{
    // They clicked Yes
	window.location = "<?php  echo $this->Html->url('/get-a-quote-non-unit-link/'.$name.'.htm'); ?>";
...
<input type="hidden" name="Amount" id="Amount" value="<?php echo $this->Session->read('Purchase.premi.total_premi'); ?>">
 <select name="bankCd" id="bankCd">
	...
	<?php if ($this->Session->read('Purchase.step1.product_id') == 24) {  // jaga motorku ?>
	 <div class="col-sm-4">
            <p style="font-size:12px;margin-top:19px;">Punya Kode Voucher?</p>
            <input  type="radio"   name="pembayaran"  value="kodevoucher" required="required" class="quote" id="inlineRadio5" /> 
            <label for="inlineRadio4">
              <span class="kk">Kode Voucher</span>
            </label>
		            <!-- <input type="text" name="KodeVoucher" id="KodeVoucher" placeholder="Kode Voucher" required />	 -->
          </div>
	<?php } ?>
</div>
    ...
    if(valQuote.form()) {
      ...
      //START: Temporary disable card input modal/alert
        ...
        sendCC();
          ...
          sendNicePay();
     }else if($('input[name=pembayaran]:checked').val()=='kodevoucher') {
         //sendKodeVoucher();
         window.location.href = "http://103.24.12.244/purchasing-promo/jaga-motorku.htm";
         <?php //$this->redirect(array('controller'=>'front','action'=>'jmk_input_vocuher')); ?>
      }
      // else { 
      //  $('#modalta').modal('show');
      // }
...
/*
  function sendKodeVoucher(){
	var kv = document.getElementById("KodeVoucher").value;

    $.ajax({
          url: "<?php echo $this->Html->url('/front/promo_kodevoucherJMK/');?>",
          type: "GET",
          cache: false,
          data: {'kodevoucher': kv},
          beforeSend: function(){ $("#paybutt").prop('disabled', true); },
          ...
          success: function(msg){  
	       //alert(msg);
          if (msg == "1"){  
                   alert("Maaf kode voucher anda tidak terdaftar, silakan menghubungi CS kami untuk informasi lebih lanjut.");
                   //location.reload();
          }else if (msg == "2"){
                   alert("Maaf kode voucher anda sudah terpakai, silakan menghubungi CS kami untuk informasi lebih lanjut.");
          }else if (msg == "3"){
                   alert("Maaf kode voucher anda sudah tidak berlaku, silakan menghubungi CS kami untuk informasi lebih lanjut.");
          }else if (msg == "0"){
                   alert("Selamat anda mendapatkan asuransi jagamotorku gratis");
          }else{
                   alert("Maaf server sibuk, mohon ulangi beberapa saat lagi.");
            //msg=JSON.parse(msg);
