...
<div class="widget-box login-front">
    ...
    <div class="row margintop">
        <div class="col-md-12">
            ...
            <hr class="redline"/>
            <!-- if gagal -->
            <?php if ($status == 'Gagal' || (!isset($status['reason_id']) || $status['reason_id'] != 1)): ?>
            ...
            if (isset($status['transactionNo'])) {
                if ($status['reason_id'] == 3)
                    echo "sudah dibayar"; else if ($status['reason_ind'] != '')
                    echo $status['reason_ind']; else
                    echo "Gagal";
            } else {
                if ($status['reason_id'] != '' && isset($status['reason_id']))
                    echo $status['reason_ind']; else
                    echo "Gagal";
            }
            ?> 
            <br>
            <?php if ($status == 'Gagal' || (!isset($status['reason_id']) || $status['reason_id'] != 1)): echo ""; ?>
            <?php else: ?>
