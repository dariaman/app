...
<div class="row margintop">
      ...
      Mohon maaf, transaksi pembayaran Anda belum berhasil. Silakan mengulangi proses pembelian <?php echo $prod ?> kembali.<br/>Klik <a href="#" onClick="backtoPaymentPage(); return false;">disini</a> untuk kembali ke halaman pembelian.
		...
		if(isset($status['transactionNo']))	{
			if($status['reason_id']==3) echo "sudah dibayar"; else if($status['reason_ind']!='') echo $status['reason_ind']; else echo "Gagal";
		} else {
			if($status['reason_id']!='' && isset($status['reason_id'])) echo $status['reason_ind']; else echo "Gagal";
		}
		?> 
				<br>
